...
export default function data({navigation}) {
    return (
        <View style={styles.container}>
                    ...
                    <View style={{flexDirection:"column",flex:1,height:"100%"}}>
                        ...
                        <View>
                            <TouchableOpacity onPress={() => Linking.openURL(".")} style={{flexDirection:"row",alignItems:"center"}}>
                                ...
                                {wp("100%")< 580 ? <Entypo name="grid" color={"white"} size={25}/> : <Text style={{fontSize:20,color:"white"}}>トップページに戻る</Text>}
                            </TouchableOpacity>  
            ...
            <TouchableOpacity style={{width:wp("100%")>800?wp("99%")/4:wp("100%"),minWidth:200}} onPress={() => window.navigator.userAgent.indexOf('iPhone')!=-1 ? Linking.openURL("https://apps.apple.com/us/app/ファミマ店員用揚げ物製作支援アプリ/id1488118221"):Linking.openURL("https://play.google.com/store/apps/details?id=familymart_toolkit.xprocess.hrkn")}>
            {wp("100%")>800?
            <Card>
                <CardItem cardBody>
                    <Image source={require("../../assets/Apps/fm.png")} style={{height: 200, width: null, flex: 1}}/>
                </CardItem>
                <CardItem footer>
                    <Text>"ファミマ店員用揚げ物製作支援アプリ"</Text>
                </CardItem>
                <CardItem footer>
                    <Text>"ファミマでバイトしている人のためのホットスナック製作支援ツールです。ホットスナックを作るタスク管理、時間を簡単に扱うことができます"</Text>
                </CardItem>
